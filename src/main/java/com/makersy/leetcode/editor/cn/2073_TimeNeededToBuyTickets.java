...
class Solution {
    public int timeRequiredToBuy(int[] tickets, int k) {
        LinkedList<Integer> personQueue = new LinkedList<>();
        for (int i = 0; i < tickets.length; i++) {
            personQueue.addLast(i);
        }
        int res = 0;
        while (tickets[k] > 0 && !personQueue.isEmpty()) {
            Integer cur = personQueue.removeFirst();
            if (tickets[cur] > 0) {
                tickets[cur]--;
                res++;
                if (tickets[cur] > 0) {
                    personQueue.addLast(cur);
                }
            }
        }
        return res;
