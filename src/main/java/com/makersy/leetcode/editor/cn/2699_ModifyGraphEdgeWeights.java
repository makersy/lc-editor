 ...
 * <p>
 * 你需要将所有边权为 -1 的边都修改为范围 [1, 2 * 10⁹] 中的 正整数 ，使得从节点 source 到节点 destination 的 最短距离
 * 为整数 target 。如果有 多种 修改方案可以使 source 和 destination 之间的最短距离等于 target ，你可以返回任意一种方案。
 * <p>
 * 如果存在使 source 到 destination 最短距离为 target 的方案，请你按任意顺序返回包含所有边的数组（包括未修改边权的边）。如果不存在这
 * 样的方案，请你返回一个 空数组 。
 * <p>
 * 注意：你不能修改一开始边权为正数的边。
 * <p>
 ...
 * <p>
 * <p>
 * Related Topics 图 最短路 堆（优先队列） 👍 86 👎 0
 */
